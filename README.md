...
For the obstruction (circle that can push string), I just calculated the normal
force using vector projection (the magnitude is a little wrong but it works). 
To apply the forces, I used Euler integration. I tried using the Velocity 
Verlet algorithm but that wasn't working, so I settled for Euler's methods. To 
keep the movement consistent (even if inaccurate) I followed
